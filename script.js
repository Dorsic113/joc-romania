...
function schimbaJucator() {
    ...
    ['landing','submeniu-trivia','submeniu-orase','submeniu-mm','ecran',
...
function ascundeToate() {
    ['landing','submeniu-orase','submeniu-mm','submeniu-trivia','submeniu-ministri','loading-screen','ecran',
...
// ── Fetch date Maramureș ───────────────────────────────────────────────────

// ── Date embedded Maramureș (fallback fără internet) ──────────────────────
...
function handleClickRO(feature, layer) {
    if (modCurent !== 'joc' || !orasCurent) return;
    const name = feature.properties.name;
...
function actualizeazaUI() {
    ...
    document.getElementById('progres-bar').style.width = ((TOTAL - ramase.length) / TOTAL * 100) + '%';
}
...
function sfarsitJoc() {
    ...
    orasCurent = null;
    resetStreakUI();
    const pct = Math.round(scor / TOTAL * 100);
    document.getElementById('final-scor').textContent    = scor + ' / ' + TOTAL;
...
function actualizeazaLandingHS() {
    ...
    const rawTrivia = lsGet('hs_trivia');
    const elTrivia  = document.getElementById('hs-trivia-landing');
    if (elTrivia) {
        if (rawTrivia) {
            const d = JSON.parse(rawTrivia);
            elTrivia.textContent = '🏆 ' + d.scor + '/10 · ' + formatTime(d.timp);
        } else {
            elTrivia.textContent = '';
        }
    }

    const rawMinistri = lsGet('hs_ministri');
    const elMinistri  = document.getElementById('hs-ministri-landing');
    if (elMinistri) {
        if (rawMinistri) {
            const d = JSON.parse(rawMinistri);
            elMinistri.textContent = '🏆 ' + d.scor + '/10 · ' + formatTime(d.timp);
        } else {
            elMinistri.textContent = '';
        }
    }
...
const TRIVIA_PERSOANE = [
    ...
    {
        nume: 'Grigore Antipa',
        info: 'biolog, naturalist • 1867–1944',
        judet: 'Iași',
        nascut: 'Iași',
        wiki: 'Grigore_Antipa'
    },
    ...
    },
    {
        ...
        judet: 'București',
        nascut: 'București',
        wiki: 'Gheorghe_Marinescu_(neurologist)'
    },
    {
...
function afiseazaHSTrivia() {
    const raw = lsGet('hs_trivia');
    const el  = document.getElementById('trivia-hs-display');
    if (raw) {
        const d = JSON.parse(raw);
        el.textContent = '🏆 Record: ' + d.scor + '/10 în ' + formatTime(d.timp);
    } else {
        el.textContent = '';
    }
}
...
function startTrivia() {
    ...
    // Amestecă și ia primele N persoane
    const amestecat = [...TRIVIA_PERSOANE].sort(() => Math.random() - 0.5);
...
function afiseazaIntrebareTrivia() {
    ...
    // Text
    document.getElementById('trivia-nume').textContent = p.nume;
    document.getElementById('trivia-info').textContent = p.info;
    document.getElementById('trivia-feedback').textContent = '';

    // Portret — fetch URL din Wikipedia REST API (cu cache)
    ...
    if (triviaImgCache[p.wiki]) {
        incarcaImaginea(triviaImgCache[p.wiki], triviaImgCache[p.wiki + '_orig']);
    } else {
        const apiUrl = 'https://en.wikipedia.org/api/rest_v1/page/summary/' + p.wiki;
        fetch(apiUrl)
            .then(r => r.ok ? r.json() : Promise.reject(r.status))
            .then(data => {
                const orig = data.thumbnail && data.thumbnail.source;
                if (orig) {
                    const url320 = /\/\d+px-/.test(orig)
                        ? orig.replace(/\/\d+px-/, '/320px-')
                        : orig;
    ...
    // Generează 4 opțiuni: corect + 3 greșite
    ...
    const cont = document.getElementById('trivia-optiuni');
    cont.innerHTML = '';
...
function raspunsTriva(btn, judetAles, persoana) {
    ...
    if (corect) {
        fb.textContent = '✅ Corect! S-a născut în ' + persoana.nascut;
    } else {
        fb.textContent = '❌ Răspuns greșit — ' + persoana.nume + ' s-a născut în ' + persoana.nascut;
    }

    setTimeout(() => {
        triviaIndex++;
        triviaAsteapta = false;
        if (triviaIndex >= TRIVIA_NR_INTREBARI) {
            sfarsitTrivia();
        } else {
            afiseazaIntrebareTrivia();
        }
...
function sfarsitTrivia() {
    ...
    if (record) {
        lsSet('hs_trivia', JSON.stringify({ scor: triviaScor, timp: timerSecunde }));
...
const MINISTRI_RO = [
    ...
    { nume: 'Dimitrie Ghica',              rol: 'Prim-ministru', ani: '1868–1870', wiki: 'Dimitrie_Ghica_(politician)',       wikiRo: 'Dimitrie_Ghica' },
    ...
    { nume: 'Ioan Emanoil Florescu',       rol: 'Prim-ministru', ani: '1876–1891', wiki: 'Ioan_Emanuel_Florescu',            wikiRo: 'Ioan_Emanoil_Florescu' },
    ...
    { nume: 'Theodor Rosetti',             rol: 'Prim-ministru', ani: '1888–1889', wiki: 'Theodor_Rosetti_(politician)',     wikiRo: 'Theodor_Rosetti' },
    ...
    { nume: 'Radu Vasile',                 rol: 'Prim-ministru',          ani: '1998–1999', wiki: 'Radu_Vasile_(politician)',            wikiRo: 'Radu_Vasile' },
    ...
    // ── Miniştri notabili ─────────────────────────────────────────────────
    { nume: 'Monica Macovei',              rol: 'Ministrul Justiției',  ani: '2004–2007', wiki: 'Monica_Macovei' },
    { nume: 'Mircea Geoană',               rol: 'Ministrul de Externe', ani: '2000–2004', wiki: 'Mircea_Geoana' },
    { nume: 'Andrei Marga',                rol: 'Ministrul Educației',  ani: '1997–2000', wiki: 'Andrei_Marga', wikiRo: 'Andrei_Marga' },
];
...
function afiseazaListaMinistri() {
    ...
    MINISTRI_RO.forEach(pm => {
        ...
        // Stocăm wiki pe rând (vizibil), nu pe img (ascuns) — fix IntersectionObserver
        div.dataset.wiki   = pm.wiki;
        div.dataset.wikiro = pm.wikiRo || '';
        const nrText = pm.nr ? pm.nr + '.' : 'int.';
        div.innerHTML =
            '<span class="ml-nr">' + nrText + '</span>' +
            '<div class="ml-img-wrap">' +
                '<span class="ml-placeholder">🧑</span>' +
                '<img class="ml-img" style="display:none" src="" alt="">' +
            '</div>' +
            '<div class="ml-info">' +
                '<div class="ml-nume">' + pm.nume + '</div>' +
                '<div class="ml-sub">' + pm.rol + ' · ' + pm.ani + '</div>' +
            '</div>';
        cont.appendChild(div);
    });

    // Observăm rândul (vizibil), nu img-ul (hidden)
    ...
    cont.querySelectorAll('.ml-rand').forEach(row => obs.observe(row));
}

function incarcaImgLista(row) {
    const wiki   = row.dataset.wiki;
    const wikiRo = row.dataset.wikiro;
    const img    = row.querySelector('.ml-img');
    const ph     = row.querySelector('.ml-placeholder');
    if (!img || !wiki) return;

    const arataImg = orig => {
        const url80 = /\/\d+px-/.test(orig) ? orig.replace(/\/\d+px-/, '/80px-') : orig;
        img.onload  = () => { img.style.display = 'block'; ph.style.display = 'none'; };
        img.onerror = () => {}; // rămâne placeholder
        img.src = url80;
    };
    const tryEn = nextFn => fetch('https://en.wikipedia.org/api/rest_v1/page/summary/' + wikiUrl(wiki))
        .then(r => r.ok ? r.json() : Promise.reject())
        .then(d => { if (d.thumbnail) arataImg(d.thumbnail.source); else nextFn(); })
        .catch(nextFn);
    const tryRo = () => {
        if (!wikiRo) return;
        fetch('https://ro.wikipedia.org/api/rest_v1/page/summary/' + wikiUrl(wikiRo))
            .then(r => r.ok ? r.json() : Promise.reject())
            .then(d => { if (d.thumbnail) arataImg(d.thumbnail.source); })
            .catch(() => {});
    };
    tryEn(tryRo);
}
...
function incarcaFotoMinistri(p) {
    ...
    const incarcaUrl   = (url320, urlOrig) => {
        img.onerror = () => {
            if (urlOrig && img.src !== urlOrig) { img.onerror = () => aratFallback(); img.src = urlOrig; }
            else aratFallback();
        };
        img.src = url320;
    };
    const proceseaza = (orig, key) => {
        const url320 = /\/\d+px-/.test(orig) ? orig.replace(/\/\d+px-/, '/320px-') : orig;
        ministriImgCache[key] = url320; ministriImgCache[key + '_orig'] = orig;
        incarcaUrl(url320, orig);
    };
    const fetchW = (url, key, next) => fetch(url)
        .then(r => r.ok ? r.json() : Promise.reject())
        .then(d => { if (d.thumbnail) proceseaza(d.thumbnail.source, key); else next(); })
        .catch(next);

    const key = p.wiki;
    if (ministriImgCache[key]) { incarcaUrl(ministriImgCache[key], ministriImgCache[key + '_orig']); return; }
    const enUrl = 'https://en.wikipedia.org/api/rest_v1/page/summary/' + wikiUrl(p.wiki);
    const fallRo = p.wikiRo
        ? () => fetchW('https://ro.wikipedia.org/api/rest_v1/page/summary/' + wikiUrl(p.wikiRo), key, aratFallback)
        : aratFallback;
    fetchW(enUrl, key, fallRo);
}
...
function afiseazaIntrebareMinistri() {
    ...
    document.getElementById('ministri-img').crossOrigin    = 'anonymous';
    document.getElementById('ministri-rol').textContent    = rolText(p);
